...
function OrderForm() {
  ...
  const handleAddOrder = (e, order_type) => {
    ...
    const tickSize = orderBooks[strike]?.tickSize || 0.05;
    ...
    if (Math.abs(price % tickSize) > 1e-10) {
      alert(`Price must be a multiple of tick size (${tickSize})`);
      return;
    }
    ...
    setPendingOrders((prev) => {
      const updated = prev.map((order) => {
        if (
          ...
          Math.abs(order.price - price) < 1e-6
        ) {
